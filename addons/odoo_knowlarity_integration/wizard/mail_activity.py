...
class MailActivity(models.Model):
    ...
    crm_lead_id = fields.Many2one('crm.lead', 'Linked Crm')
    ...
    def write(self, values):
        if 'date_deadline' in values:
            self.mapped('crm_lead_id').write({'date_deadline': values['date_deadline']})
        ...
        for index, activity in enumerate(self):
            if activity.crm_lead_id:
                phonecall_values_to_keep[index] = {
                    'note': activity.note,
                    'crm_lead_id': activity.crm_lead_id,
                    'call_date': activity.crm_lead_id.call_date,
        ...
        if phonecall_values_to_keep:
            ...
            for index, message in enumerate(messages):
                if index in phonecall_values_to_keep:
                    ...
                    phonecall = values_to_keep['crm_lead_id']
                    values_to_write = {
                        # 'state': 'done',
